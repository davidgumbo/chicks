...
class PoultryControllerDashboard(http.Controller):
    ...
    def get_poultry_count(self):
        ...
        if not start_date_str or not end_date_str:
            ...
            try:
                ...
                try:
                    ...
                    # If still fails, use default dates
                    end_date = datetime.today()
                    start_date = end_date - timedelta(days=30)
        ...
        farm_house_count = request.env['chicken.farm'].sudo().search_count([
            ('states', '=', 'approve'),
        ])

        inward_farm_house_count = request.env['chicken.house'].sudo().search_count([
        ...
        main_product_count = request.env['product.product'].sudo().search_count([
            ('is_chicken', '=', True),
        ])

        final_product_count = request.env['product.product'].sudo().search_count([
            ('last_product', '=', True),
        ])
    ...
    def _get_animal_dashboard_values(self):
        ...
        if not start_date_str or not end_date_str:
            ...
            try:
                ...
                end_date = datetime.strptime(end_date_str, '%Y-%m-%d %H:%M:%S.%f').date()
            except ValueError:
                try:
                    # Fall back to parsing without microseconds
        ...
        while current_date <= end_date:
            ...
            inward_monthly_data.append([month_name, monthly_inward])

            # Expenses data
            monthly_expenses = sum(
                1 for expense in expenses_data
                if expense.today_date and first_day <= expense.today_date <= last_day
            )
            expenses_monthly_data.append([month_name, monthly_expenses])
